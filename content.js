...
async function hideVisitedResults() {
  ...
  console.log('DeadDupe: isEnabled =', isEnabled, 'visitedUrls count =', visitedUrls.length);
  ...
  console.log('DeadDupe: Found', searchLinks.length, 'search result links');
  ...
  searchLinks.forEach(({ url, container }) => {
    ...
    if (normalizedUrl && visitedUrls.includes(normalizedUrl)) {
      ...
      container.setAttribute('data-deaddupe-hidden', 'true');
      hiddenCount++;
      console.log('DeadDupe: Hiding', normalizedUrl);
    } else {
      // Make sure it's visible
      if (container.hasAttribute('data-deaddupe-hidden')) {
        container.style.display = '';
        container.removeAttribute('data-deaddupe-hidden');
      }
    }
  });
  
  console.log(`DeadDupe: Hid ${hiddenCount} previously visited result(s)`);
}

// Function to show all results (when extension is disabled)
function showAllResults() {
  const hiddenResults = document.querySelectorAll('[data-deaddupe-hidden]');
  hiddenResults.forEach((result) => {
    result.style.display = '';
    result.removeAttribute('data-deaddupe-hidden');
  });
}
